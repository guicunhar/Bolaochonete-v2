...
app.post('/api/first-login', async (req, res) => {
  try {
    ...
    await run(
      `UPDATE users SET password_hash=?, is_precadastro=0, champion_pick=?, best_player_pick=?, top_scorer_pick=?, bonus_locked=1 WHERE id=?`,
      [hash, champion_pick || null, best_player_pick || null, top_scorer_pick || null, user.id]
    );
